...
export const PortfolioParallax: React.FC<ItemPortfolioProps> = ({ data, even }) => {
        ...
        >
          ...
          <div className="flex-1 flex md:px-4 lg:px-6 items-center">
            <div className={`flex flex-1 flex-wrap  ${even ? 'md:justify-end md:text-right' : ''}`}>
              <h3 className="text-color-1 text-5xl font-black to-up">{data.frontmatter.title}</h3>
              <p className="lg:mt-4 to-up">{data.frontmatter.description}</p>
              <Button
                to={data.fields.slug}
                title={`View ${data.frontmatter.title}`}
